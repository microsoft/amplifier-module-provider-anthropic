...
Verifies that mount() registers:
  ...
  - a lazy contributor callback on session.cost channel under name 'provider-anthropic'
"""
from decimal import Decimal

import pytest

from amplifier_module_provider_anthropic import mount
...
async def test_contributor_registered_at_mount(mock_coordinator):
    """mount() must register a contributor on ('session.cost', 'provider-anthropic')."""
    await mount(mock_coordinator, config={})
    assert ("session.cost", "provider-anthropic") in mock_coordinator.registered_contributors
